...
* **Delimitador de Fin de Cadena:** $\text{\$}$
...
* **Resultado:**
  $$PRIM(S) = \{\mathbf{dos}, \mathbf{tres}, \mathbf{cuatro}, \mathbf{cinco}, \mathbf{uno}\}$$
...
Formalmente se define:
$$SIG(X) = \{a \in V_T \mid S \Rightarrow^* \alpha X a \beta\} \cup \{\text{\$} \mid S \Rightarrow^* \alpha X\}$$

Las reglas sistemáticas aplicadas en la deducción son:
1. **Condición de frontera:** El marcador de fin de cadena pertenece obligatoriamente al símbolo inicial: $\text{\$} \in SIG(S)$.
...
#### Análisis para el Símbolo Inicial $S$
* Por definición formal, se incorpora el fin de entrada: $\text{\$} \in SIG(S)$.
